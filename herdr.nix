...

{
  ...
  options.gentleman.herdr.worktreeDirectory = lib.mkOption {
    type = lib.types.nullOr lib.types.str;
    default = null;
    description = "Optional default directory for Herdr worktrees.";
  };
  ...
  config = {
    ...
    home.activation.installHerdr = lib.hm.dag.entryAfter [ "linkGeneration" ] ''
      ...
      ${lib.optionalString (config.gentleman.herdr.worktreeDirectory != null) ''
        cat >> "$HOME/.config/herdr/config.toml" <<'EOF'

        [worktrees]
        directory = ${builtins.toJSON config.gentleman.herdr.worktreeDirectory}
        EOF
      ''}
